...
void proc_main_task(s32 taskId)
{
    ...
    u32 rssi = 0;
    Timer_Init();
    // Ql_Timer_RegisterFast(999, Callback_Timer, NULL);
    // Ql_Timer_Start(999, 2000, TRUE);
    // init_cts_button();
    ...
    semOLED   =  Ql_OS_CreateSemaphore("semOLED", 0);
...
void proc_oled(s32 taskId) {
    ...
    while (1) {
        ...
        //    APP_DEBUG("proc_oled LeftStackSize:%d\r\n", Ql_OS_GetCurrenTaskLeftStackSize());
        Ql_Sleep(50);
    }
}
...
void cts_eint_callback(Enum_PinName eintPinName, Enum_PinLevel pinLevel, void* customParam)
{
    s32 ret;
       if (PINNAME_CTS == eintPinName) {
...
void init_cts_button(void)
{
    s32 ret;
    static s32 ctsCounter;
    ret = Ql_EINT_Register(PINNAME_CTS, cts_eint_callback, (void *)&ctsCounter);
    ret = Ql_EINT_Init(PINNAME_CTS, EINT_LEVEL_TRIGGERED, 0, 5, 1);
}
